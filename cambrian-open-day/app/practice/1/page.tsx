...
export default function ArrayReversalProblem() {
  const router = useRouter()
  const problem = practiceProblems[0] // Array Reversal problem
  ...
  const handleRunCode = () => {
    ...
    setTimeout(() => {
      if (code.includes("for") && code.includes("arr.length - 1") && code.includes("push")) {
        setStatus("success")
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-yellow-100/20 dark:bg-yellow-900/10 border-yellow-200 dark:border-yellow-800/30">
              <CardContent className="p-4 flex items-center space-x-4">
                ...
                <div>
                  <h3 className="font-medium text-yellow-800 dark:text-yellow-300">Hint from Pixel Friend</h3>
                  <p className="text-sm text-yellow-700 dark:text-yellow-400">
                    {attempts === 0
                      ? "Good luck with this problem! I'm here if you need help."
                      : attempts === 1
                        ? "Think about how to traverse the array in reverse order."
                        : "Remember to create a new array to store your results!"}
                  </p>
                </div>
              </CardContent>
            </Card>
